...
void MapView::loadFile(QString name){
    ...
    if(QFile::exists(file_path)){
        file_velocity = cv::imread(file_path.toStdString(), cv::IMREAD_GRAYSCALE);
...
void MapView::setMapSize(int width, int height){
    qDebug() << "setmapsize" << canvas_width;
    canvas_width = width;
...
void MapView::setMap(){
    map = QPixmap::fromImage(mat_to_qimage_cpy(map_orin));
...
//    qDebug() << QString().sprintf("%f, xy(%d, %d), xxyy(%f, %f), newxy(%f, %f), draw_xy(%d, %d), draw_width(%d)",scale, x, y, xx, yy, newx, newy, draw_x, draw_y, draw_width);
