...
void LCMHandler::setData(ST_MAP _map, ST_ROBOT _robot){
    map = _map;
    robot = _robot;
}
...
ST_FPOINT LCMHandler::setAxis(ST_FPOINT _point){
    ST_FPOINT temp;
    temp.x = -_point.y;
    temp.y = -_point.x;
    return temp;
}
ST_FPOINT LCMHandler::canvasTomap(int x, int y){
    ST_FPOINT temp;
//    qDebug() << map.gridwidth << map.origin[0] << map.origin[1];
//    qDebug() << x << y;
